...
##usage: ./modify_netlist.sh <netlist_filename>


#set filename_in = "s832.v"
set filename_in = $argv[1]
set filename_out = "${filename_in}.modify"
set temp =  "${filename_in}_temp"
rm -rf $filename_out
rm -rf $temp

##delete the D_FF module part
##warning some modules do not have D_FF module part
set exist = `cat ${filename_in} | grep -e 'module.*dff'`
#echo $exist
if ( "$exist" != "" ) then
...
cat temp| sed -e 's/GND//g'| sed -e 's/VDD//g'| sed -e 's/CK//g' | sed '/^$/d'> $filename_out
